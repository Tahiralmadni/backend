...
// Add this function to mark holidays for all teachers
exports.processHolidays = async (req, res) => {
  try {
    ...
    // Check if the request is for all teachers or specific teacher
    const teacherId = req.params.teacherId;
    
    console.log(`Processing holidays from ${startDate} to ${endDate} for ${teacherId || 'all teachers'}`);
    
    let teachers;
    
    if (teacherId && teacherId !== 'all-teachers') {
      // Get specific teacher
      teachers = await Teacher.find({ _id: teacherId, active: true });
    } else {
      // Get all active teachers
      teachers = await Teacher.find({ active: true });
    }
    ...
    // Get the holiday dates from the request
    // In a real app, this would come from a proper holiday API or Google Calendar
    const holidayDates = req.body.holidays || [];
    
    // Process each teacher for each holiday
    const results = [];
    ...
    for (const teacher of teachers) {
      for (const holiday of holidayDates) {
        ...
        // Check if attendance record already exists for this date and teacher
        ...
        if (!existingRecord) {
          // Create new holiday attendance record
          const attendance = new Attendance({
            teacher: teacher._id,
            teacherId: teacher._id,
            date: new Date(date),
            status: 'holiday',
            comment: `Holiday: ${name || 'Unspecified Holiday'}`,
            autoGenerated: true,
            workHours: 0,
            salaryDeduction: 0
          });
    ...
    res.status(500).json({
      message: 'Server error during holiday processing',
      error: error.message
    });
  }
}; 